<template>
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Dodaj urządzenie</h1>
          <p class="text-slate-400 mt-1">Uzupełnij dane urządzenia. Wymagana jest tylko nazwa.</p>
        </div>
      ...
      <div class="card">
        <form class="grid gap-4" @submit.prevent="onSubmit">
          ...
          <div class="grid gap-2">
            ...
            <input v-model.trim="form.name" class="input" placeholder="np. Laptop do pracy" required />
          </div>
  
          <div class="grid md:grid-cols-2 gap-4">
            <div class="grid gap-2">
              <label class="label">Marka</label>
              <input v-model.trim="form.brand" class="input" placeholder="np. Apple" />
            </div>
            <div class="grid gap-2">
              <label class="label">Model</label>
              <input v-model.trim="form.model" class="input" placeholder="np. MacBook Air M2" />
            </div>
          </div>
          ...
          <div class="grid md:grid-cols-2 gap-4">
            <div class="grid gap-2">
              ...
              <input v-model.trim="form.serial_number" class="input" placeholder="np. C02..." />
            </div>
            <div class="grid gap-2">
          ...
          <div class="grid gap-2">
            ...
            <textarea v-model.trim="form.notes" class="textarea" rows="4" placeholder="np. gwarancja, konfiguracja..."></textarea>
          </div>
  
          <div class="flex items-center gap-3 pt-2">
            <button class="btn btn-primary" :disabled="loading">
              <span v-if="loading">Zapisywanie...</span>
              <span v-else>Zapisz urządzenie</span>
            </button>
  ...
  async function onSubmit() {
    ...
    try {
      ...
      success.value = "Urządzenie dodane ✅";
